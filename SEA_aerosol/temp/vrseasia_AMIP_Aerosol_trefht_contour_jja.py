...
def plotalldiff(lons,lats,res1,tt1,clim1,res2,tt2,clim2,res3,tt3,clim3,opt):
    ...
    ax1.set_title(r'$\Delta_{total}$ '+varname+', regional mean='+str(round(np.mean(res1),4))+var_unit,fontsize=5,pad=3)
...
#    ax1.set_ylabel("Latitudes [degrees]",fontsize=5)
    ...
    ax2.set_title(r'$\Delta_{fast}$ '+varname+', regional mean='+str(round(np.mean(res2),4))+var_unit,fontsize=5,pad=3)
    map = Basemap(projection='cyl',llcrnrlat=latbounds[0],urcrnrlat=latbounds[1],llcrnrlon=lonbounds[0],urcrnrlon=lonbounds[1],resolution = 'l')
    map.drawcoastlines()
    map.drawcountries()
    parallels = np.arange(latbounds[0], latbounds[1],20)
...
#    ax2.set_ylabel("Latitudes [degrees]",fontsize=5)
    ...
    ax3.set_title(r'$\Delta_{slow}$ '+varname+', regional mean='+str(round(np.mean(res3),4))+var_unit,fontsize=5,pad=3)
    map = Basemap(projection='cyl',llcrnrlat=latbounds[0],urcrnrlat=latbounds[1],llcrnrlon=lonbounds[0],urcrnrlon=lonbounds[1],resolution = 'l')
    map.drawcoastlines()
    map.drawcountries()
    parallels = np.arange(latbounds[0], latbounds[1],20)
...
#    fig.subplots_adjust(right=0.7,hspace = 0.15)
    ...
    if (opt==1):
        plt.savefig(outdir+"vrseasia_aerosol_amip_jja_"+varfname+"_SEA_contour_response_with_siglev_aerosolsinone.pdf",bbox_inches='tight')
    else:
        plt.savefig(outdir+"vrseasia_aerosol_amip_jja_"+varfname+"_SEA_contour_response_aerosolsinone.pdf",bbox_inches='tight')
