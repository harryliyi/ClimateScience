...
percentile_ranges = [40, 75, 90, 95, 99]
percentile_tops = [60, 85, 93, 97, 99.5]
percentile_bots = [0, 60, 85, 93, 97]
...
def plotdiff(lons, lats, res_total, tt_total, res_thermo, tt_thermo, res_dyn, tt_dyn, forcingstr, forcingfname, opt):
    fig = plt.figure()
    # total response
    ax1 = fig.add_subplot(311)
    ax1.set_title('Total response, mean='+str(round(np.mean(res_total), 3))+var_unit, fontsize=7, pad=3)
...
#    print(tt_total)
    ...
    ax2.set_title('Thermodynamic response, mean='+str(round(np.mean(res_thermo), 3))+var_unit, fontsize=7, pad=3)
    map = Basemap(projection='cyl', llcrnrlat=latbounds[0], urcrnrlat=latbounds[1],
                  llcrnrlon=lonbounds[0], urcrnrlon=lonbounds[1], resolution='l')
    map.drawcoastlines(linewidth=0.3)
    map.drawcountries()
    ...
    ax3.set_title('Dynamic response, mean='+str(round(np.mean(res_total), 3))+var_unit, fontsize=7, pad=3)
    map = Basemap(projection='cyl', llcrnrlat=latbounds[0], urcrnrlat=latbounds[1],
                  llcrnrlon=lonbounds[0], urcrnrlon=lonbounds[1], resolution='l')
    map.drawcoastlines(linewidth=0.3)
    map.drawcountries()
...
# plot for convergence decomposition, div(qu), qdiv(u), udiv(q), residual
def plotconv(lons, lats, mfc, msc, mtc, percentile, titlestr, fname, outdir):
    fig = plt.figure()
    # mfc
    ax1 = fig.add_subplot(221)
    ax1.set_title('a) Moisture flux convergence', fontsize=7, pad=3)
    map = Basemap(projection='cyl', llcrnrlat=latbounds[0], urcrnrlat=latbounds[1],
                  llcrnrlon=lonbounds[0], urcrnrlon=lonbounds[1], resolution='l')
    map.drawcoastlines(linewidth=0.3)
    map.drawcountries()
    ...
    mlons, mlats = np.meshgrid(lons, lats)
    x, y = map(mlons, mlats)
    clevs = np.arange(-50., 50.1, 10.)
    cs = map.contourf(x, y, mfc, clevs, cmap=cm.BrBG, alpha=0.9, extend="both")

    # product of moisture and mass convergence
    ax2 = fig.add_subplot(222)
    ax2.set_title('b) Product of moisture and mass convergence', fontsize=7, pad=3)
    map = Basemap(projection='cyl', llcrnrlat=latbounds[0], urcrnrlat=latbounds[1],
                  llcrnrlon=lonbounds[0], urcrnrlon=lonbounds[1], resolution='l')
    map.drawcoastlines(linewidth=0.3)
    map.drawcountries()
    ...
    map.drawmeridians(meridians, labels=[0, 0, 0, 1], fontsize=4, linewidth=0.1)
    clevs = np.arange(-50., 50.1, 10.)
    cs = map.contourf(x, y, msc, clevs, cmap=cm.BrBG, alpha=0.9, extend="both")

    # advection of moiture
    ax3 = fig.add_subplot(223)
    ax3.set_title('c) Advection of moiture', fontsize=7, pad=3)
    map = Basemap(projection='cyl', llcrnrlat=latbounds[0], urcrnrlat=latbounds[1],
                  llcrnrlon=lonbounds[0], urcrnrlon=lonbounds[1], resolution='l')
    map.drawcoastlines(linewidth=0.3)
    map.drawcountries()
    ...
    map.drawmeridians(meridians, labels=[0, 0, 0, 1], fontsize=4, linewidth=0.1)
    clevs = np.arange(-50., 50.1, 10.)
    cs = map.contourf(x, y, mtc, clevs, cmap=cm.BrBG, alpha=0.9, extend="both")
    ...
    map.drawmeridians(meridians, labels=[0, 0, 0, 1], fontsize=4, linewidth=0.1)
    clevs = np.arange(-50., 50.1, 10.)
    cs = map.contourf(x, y, mfc-msc-mtc, clevs, cmap=cm.BrBG, alpha=0.9, extend="both")
...
#    cbar = fig.colorbar(cs,orientation='horizontal',fraction=0.15, aspect= 25,shrink = 0.8)
    ...
    # add title
    plt.savefig(outdir+"vrseasia_aerosol_amip_jja_"+str(percentile)+"th_extreme_"+varfname +
                "_SEA_contour_mfc_decompose_"+fname+".png", dpi=600, bbox_inches='tight')
    plt.suptitle(titlestr+" JJA "+str(percentile)+"th extreme "+varstr+" Moisture Budget", fontsize=9, y=0.95)
    plt.savefig(outdir+"vrseasia_aerosol_amip_jja_"+str(percentile)+"th_extreme_" +
                varfname+"_SEA_contour_mfc_decompose_"+fname+".pdf", bbox_inches='tight')
    plt.close(fig)


# plot for eddy decomposition, P, dq/dt, div(qu), residual
def ploteddy(lons, lats, prect, dq, mfc, percentile, titlestr, fname, outdir):
    fig = plt.figure()
    # total precipitation
    ax1 = fig.add_subplot(221)
    ...
    # Moisture-storage
    ax2 = fig.add_subplot(222)
    ax2.set_title('b) Moisture-storage', fontsize=7, pad=3)
    map = Basemap(projection='cyl', llcrnrlat=latbounds[0], urcrnrlat=latbounds[1],
                  llcrnrlon=lonbounds[0], urcrnrlon=lonbounds[1], resolution='l')
    map.drawcoastlines(linewidth=0.3)
    map.drawcountries()
    ...
    map.drawmeridians(meridians, labels=[0, 0, 0, 1], fontsize=4, linewidth=0.1)
    clevs = np.arange(-50., 50.1, 10.)
    cs = map.contourf(x, y, dq, clevs, cmap=cm.BrBG, alpha=0.9, extend="both")
    ...
    map.drawmeridians(meridians, labels=[0, 0, 0, 1], fontsize=4, linewidth=0.1)
    clevs = np.arange(-50., 50.1, 10.)
    cs = map.contourf(x, y, prect-dq-mfc, clevs, cmap=cm.BrBG, alpha=0.9, extend="both")
...
#    cbar = fig.colorbar(cs,orientation='horizontal',fraction=0.15, aspect= 25,shrink = 0.8)
    ...
    # add title
    plt.savefig(outdir+"vrseasia_aerosol_amip_jja_"+str(percentile)+"th_extreme_"+varfname +
                "_SEA_contour_eddy_decompose_"+fname+".png", dpi=600, bbox_inches='tight')
    plt.suptitle(titlestr+" JJA "+str(percentile)+"th extreme "+varstr+" Moisture Budget", fontsize=9, y=0.95)
    plt.savefig(outdir+"vrseasia_aerosol_amip_jja_"+str(percentile)+"th_extreme_" +
                varfname+"_SEA_contour_eddy_decompose_"+fname+".pdf", bbox_inches='tight')
    plt.close(fig)


# plot for moisture budget, P, dq/dt, qdiv(u), udiv(q)
def plotmoist(lons, lats, prect, dq, msc, mtc, percentile, titlestr, fname, outdir):
    fig = plt.figure()
    # total precipitation
    ax1 = fig.add_subplot(221)
    ax1.set_title('a) Total precipitation', fontsize=7, pad=3)
    ...
    # Product of moisture and mass convergence, qdiv(u)
    ax3 = fig.add_subplot(223)
    ax3.set_title('c) Product of moisture and mass convergence', fontsize=7, pad=3)
    map = Basemap(projection='cyl', llcrnrlat=latbounds[0], urcrnrlat=latbounds[1],
                  llcrnrlon=lonbounds[0], urcrnrlon=lonbounds[1], resolution='l')
    map.drawcoastlines(linewidth=0.3)
    map.drawcountries()
    ...
    # Advection of moisture, udiv(q)
    ax4 = fig.add_subplot(224)
    ax4.set_title('d) Advection of moisture', fontsize=7, pad=3)
    map = Basemap(projection='cyl', llcrnrlat=latbounds[0], urcrnrlat=latbounds[1],
                  llcrnrlon=lonbounds[0], urcrnrlon=lonbounds[1], resolution='l')
    map.drawcoastlines(linewidth=0.3)
    map.drawcountries()
...
#    cbar = fig.colorbar(cs,orientation='horizontal',fraction=0.15, aspect= 25,shrink = 0.8)
    ...
    # add title
    plt.savefig(outdir+"vrseasia_aerosol_amip_jja_"+str(percentile)+"th_extreme_"+varfname +
                "_SEA_contour_moisture_budget_"+fname+".png", dpi=600, bbox_inches='tight')
    plt.suptitle(titlestr+" JJA "+str(percentile)+"th extreme "+varstr+" Moisture Budget", fontsize=9, y=0.95)
    plt.savefig(outdir+"vrseasia_aerosol_amip_jja_"+str(percentile)+"th_extreme_" +
                varfname+"_SEA_contour_moisture_budget_"+fname+".pdf", bbox_inches='tight')
    plt.close(fig)
...
for idx_percent, percentile in enumerate(percentile_ranges):
    ...
    # pre_percent1 = np.broadcast_to(pre_percent1, (ntime, nlats, nlons))
    # pre_percent2 = np.broadcast_to(pre_percent2, (ntime, nlats, nlons))
    # pre_percent3 = np.broadcast_to(pre_percent3, (ntime, nlats, nlons))
    # pre_percent4 = np.broadcast_to(pre_percent4, (ntime, nlats, nlons))
    # pre_percent5 = np.broadcast_to(pre_percent5, (ntime, nlats, nlons))

    # print(pre_percent1[0, 0, :])
    # print(pre_percent1[:, 0, 0])
    ...
    print('Plotting Moisture budget for extreme events...')

    fname = "case1"
    titlestr = case1
    ploteddy(lons, lats, pre_ext1, dq_ext1, mfc_ext1, percentile, titlestr, fname, outdir_percent)
    plotconv(lons, lats, mfc_ext1, msc_ext1, mtc_ext1, percentile, titlestr, fname, outdir_percent)
    plotmoist(lons, lats, pre_ext1, dq_ext1, msc_ext1, mtc_ext1, percentile, titlestr, fname, outdir_percent)

    fname = "case2"
    titlestr = case2
    ploteddy(lons, lats, pre_ext2, dq_ext2, mfc_ext2, percentile, titlestr, fname, outdir_percent)
    plotconv(lons, lats, mfc_ext2, msc_ext2, mtc_ext2, percentile, titlestr, fname, outdir_percent)
    plotmoist(lons, lats, pre_ext2, dq_ext2, msc_ext2, mtc_ext2, percentile, titlestr, fname, outdir_percent)

    fname = "case3"
    titlestr = case3
    ploteddy(lons, lats, pre_ext3, dq_ext3, mfc_ext3, percentile, titlestr, fname, outdir_percent)
    plotconv(lons, lats, mfc_ext3, msc_ext3, mtc_ext3, percentile, titlestr, fname, outdir_percent)
    plotmoist(lons, lats, pre_ext3, dq_ext3, msc_ext3, mtc_ext3, percentile, titlestr, fname, outdir_percent)

    fname = "case4"
    titlestr = case4
    ploteddy(lons, lats, pre_ext4, dq_ext4, mfc_ext4, percentile, titlestr, fname, outdir_percent)
    plotconv(lons, lats, mfc_ext4, msc_ext4, mtc_ext4, percentile, titlestr, fname, outdir_percent)
    plotmoist(lons, lats, pre_ext4, dq_ext4, msc_ext4, mtc_ext4, percentile, titlestr, fname, outdir_percent)

    fname = "case5"
    titlestr = case5
    ploteddy(lons, lats, pre_ext5, dq_ext5, mfc_ext5, percentile, titlestr, fname, outdir_percent)
    plotconv(lons, lats, mfc_ext5, msc_ext5, mtc_ext5, percentile, titlestr, fname, outdir_percent)
    plotmoist(lons, lats, pre_ext5, dq_ext5, msc_ext5, mtc_ext5, percentile, titlestr, fname, outdir_percent)
