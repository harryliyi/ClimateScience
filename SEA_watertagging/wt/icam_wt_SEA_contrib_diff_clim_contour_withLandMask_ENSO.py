...
outdir = '/scratch/d/dylan/harryli/gpcdata/Analysis/icam5/SEA_wt_1920today/annual_variability/watertag/new_py_precent_ENSOdiff_contour_wtLandMask_10N_20N/'
...
for m in range(0, 12):
    for y in range(year1, year2 + 1):
        ...
        if np.in1d(y, years_elweak):
            ...
            globalPrecip_el[m] = globalPrecip_el[m] + np.sum(h20step)
            for s in range(len(SOURCES)):
                for tag in range(5, 5+tagnum):
                    ...
                    localPrecip_el[tagind, m] = localPrecip_el[tagind, m] + np.sum(tagstep)

        if np.in1d(y, years_laweak):
            print('Current year ('+str(y)+') is a La Nina year')
            h20step = filename.variables['PRECT'][0, la_drc_1:la_drc_2 + 1, lo_drc_1:lo_drc_2 + 1]
            globalPrecip_la[m] = globalPrecip_la[m] + np.sum(h20step)
            for s in range(len(SOURCES)):
                for tag in range(5, 5+tagnum):
                    tagind = tag - 5
                    tagstep = filename.variables[sourceL[s] + 'wt' +
                                                 str(tag) + sourceS[s]][0, la_drc_1:la_drc_2 + 1, lo_drc_1:lo_drc_2 + 1]
                    localPrecip_la[tagind, m] = localPrecip_la[tagind, m] + np.sum(tagstep)
...
legends = ['a) Under El Nino events', 'b) Under La Nina events', 'c) Differences in ENSO events']
...
for mm in range(0, 12):
    ...
    for t in range(0, tagnum):
        localratio_el[t] = 100.0 * localPrecip_ael[t] / globalPrecip_ael
        print(str(t+5)+': '+str(localratio_el[t]))
    for t in range(0, tagnum):
        localratio_la[t] = 100.0 * localPrecip_ala[t] / globalPrecip_ala
    ...
    for ss in range(3):
        ...
        if ss < 2:
            csm = m.pcolormesh(x, y, plot_data[ss], cmap=colors[ss], vmax=20., vmin=1., ax=axes[ss])
            ...
            csm = m.pcolormesh(x, y, plot_data[ss], cmap=colors[ss], vmax=10., vmin=-10., ax=axes[ss])
            cbar = fig.colorbar(csm, shrink=0.8, pad=0.03, orientation='vertical', ax=axes[ss])
            cbar.set_ticks(np.arange(-10, 12.5, 2.5))
            cbar.set_ticklabels(np.arange(-10, 12.5, 2.5))
        cbar.ax.tick_params(labelsize=3)
        cbar.ax.set_title('%', fontsize=4)
    # drawMapRect(las, los, m, 'white')
    # plt.show()
    plt.savefig(outdir+'icam_SEA_'+str(year1)+'_'+str(year2)+'_percent_contrib_clim_contour_withLandMask_' +
