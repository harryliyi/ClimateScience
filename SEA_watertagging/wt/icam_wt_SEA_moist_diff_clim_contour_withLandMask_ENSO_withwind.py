...
for mm in range(0, 12):
    ...
    select_el = np.in1d(model_time.month, monthts[mm]) & np.in1d(model_time.year, years_elweak)
    select_la = np.in1d(model_time.month, monthts[mm]) & np.in1d(model_time.year, years_laweak)
    ...
    # plt.show()
    plt.savefig(outdir+'icam_SEA_'+str(year1)+'_'+str(year2)+'_moist_contrib_clim_contour_withLandMask_wtwind_' +
