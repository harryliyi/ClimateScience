...
# setup directory
############################################################################
outdir = '/scratch/d/dylan/harryli/cesm1/vrcesm/Analysis/vrseasia_AMIP_1979_to_2005/pre/obs/CPC/clustering/'
...
ncluster = 4
fname = 'CPC_precip_kmeans_nclusters'
outdir2 = '/scratch/d/dylan/harryli/cesm1/vrcesm/Analysis/vrseasia_AMIP_1979_to_2005/pre/obs/CPC/clustering/' + \
...
# read CPC
var, time, lats, lons = readobs_pre_CPC('precip', iniyear, endyear, frequency, latbounds, lonbounds)
...
var_mask = dataset_var[0, :, :].mask
block_ts = []
print(np.sum(var_mask))
for idx in range(dataset_var.shape[0]):
    if (np.sum(np.logical_xor(var_mask, dataset_var[idx, :, :].mask)) != 0):
        print(str(time[idx])+' has a different mask with initial mask, will be blocked')
        print('The number of different grids are ' + str(np.sum(np.logical_xor(var_mask, dataset_var[idx, :, :].mask))))
        block_ts.append(idx)
...
if len(block_ts) > 0:
    # print(len(block_ts))
    original_time = time
    dataset_var = np.delete(dataset_var, block_ts, 0)
    time = np.delete(time, block_ts, 0)
...
print(np.sum(np.isnan(temp_var)))
...
for icluster in nclusters_list:
    ...
    # kmeans = KMeans(n_clusters=icluster, init=pca.components_, n_init=1, max_iter=10000).fit(temp_var)

    # use pca to compress data
    pca_data = PCA(n_components=k).fit_transform(temp_var)
    print(pca_data.shape)
    kmeans = KMeans(n_clusters=icluster, init='k-means++', n_init=300, max_iter=2000).fit(pca_data)
...
for icluster in range(ncluster):
    ...
    title = str(iniyear)+' to '+str(endyear)+' CPC Monthly mean precip TS in the group: '+str(icluster)
    fname = 'CPC_prect_monthly_mean_ts_group_'+str(icluster)+'.png'
...
title = str(iniyear)+' to '+str(endyear)+' CPC Monthly mean precip TS'
fname = 'CPC_prect_monthly_mean_ts_groupinone.png'
...
for icluster in range(ncluster):
    ...
    title = str(iniyear)+' to '+str(endyear)+' CPC Annual mean precip TS in the group: '+str(icluster)
    fname = 'CPC_prect_annual_mean_ts_group_'+str(icluster)+'.png'
...
title = str(iniyear)+' to '+str(endyear)+' CPC Annual mean precip TS'
fname = 'CPC_prect_annual_mean_ts_groupinone.png'
...
for icluster in range(ncluster):
    ...
    title = str(iniyear)+' to '+str(endyear)+' CPC Total precip distribution in the group: '+str(icluster)
    fname = 'CPC_prect_hist_group_'+str(icluster)+'.png'
...
title = str(iniyear)+' to '+str(endyear)+' CPC Total precip distribution'
fname = 'CPC_prect_hist_groupinone.png'
...
for icluster in range(ncluster):
    ...
    title = str(iniyear)+' to '+str(endyear)+' CPC Seasonal cycle of precip in the group: '+str(icluster)
    fname = 'CPC_prect_clim_mean_line_group_'+str(icluster)+'.png'
...
title = str(iniyear)+' to '+str(endyear)+' CPC Seasonal cycle of precip'
fname = 'CPC_prect_clim_mean_line_groupinone.png'
...
# test pickle data
pickle.dump(kmeans_res, open(outdir+'CPC_kmeans_result_overland_'+str(ncluster)+'cluster.p', "wb"))
res_load = pickle.load(open(outdir+'CPC_kmeans_result_overland_'+str(ncluster)+'cluster.p', "rb"))
