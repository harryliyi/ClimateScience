...
from modules.datareader.mod_dataread_obs_pre import readobs_pre_day
...
# setup directory
############################################################################
outdir = '/scratch/d/dylan/harryli/cesm1/vrcesm/Analysis/vrseasia_AMIP_1979_to_2005/pre/VRseasia_vs_CORDEX_SEA/APHRODITE/extremes/'
...
obs_var, obs_time, obs_lats, obs_lons = readobs_pre_day(project, iniyear, endyear, latbounds, lonbounds)

obs_var[obs_var.mask] = np.nan
print(obs_var[0, :, :])

legends = ['CESM-vrseasia', 'CESM-ne30', 'CESM-fv0.9x1.25', 'CESM-fv1.9x2.5', 'CORDEX-ICHEC-EC-EARTH',
           'CORDEX-IPSL-IPSL-CM5A-LR', 'CORDEX-MPI-M-MPI-ESM-MR', 'CORDEX-MOHC-HadGEM2-ES', 'APHRODITE']
colors = ['red', 'yellow', 'green', 'blue', 'tomato', 'goldenrod', 'darkcyan', 'darkmagenta', 'black']
line_types = ['dashed', 'dashed', 'dashed', 'dashed', '-.', '-.', '-.', '-.', '-']

cesm_legends = ['CESM-vrseasia', 'CESM-ne30', 'CESM-fv0.9x1.25', 'CESM-fv1.9x2.5', 'APHRODITE']
...
print('Plot the precip histogram for model and APHRODITE')
fig = plt.figure()
...
fname = 'vrcesm_all_prect_hist_vs_cordex_refAPHRODITE.pdf'
...
print('Plot the histogram of precip over threshold for model and APHRODITE')
fig = plt.figure()
ax = fig.add_subplot(111)
...
fname = 'vrcesm_prect_over_'+str(premin)+'mm_hist_vs_cordex_refAPHRODITE.pdf'
...
# test the mean residual life plot for APHRODITE
...
fname = 'APHRODITE_prect_'+str(percentile)+'th_mean_residual_life_plot.pdf'
plt.savefig(outdir+fname, bbox_inches='tight')
plt.close(fig)
...
title = 'Estimated parameter (Xi) for APHRODITE'
plt.suptitle(title, fontsize=10, y=0.95)

fname = 'APHRODITE_prect_'+str(percentile)+'th_xi_plot.pdf'
plt.savefig(outdir+fname, bbox_inches='tight')
plt.close(fig)
...
title = 'Estimated parameter (sigma) for APHRODITE'
plt.suptitle(title, fontsize=10, y=0.95)

fname = 'APHRODITE_prect_'+str(percentile)+'th_sigma_plot.pdf'
plt.savefig(outdir+fname, bbox_inches='tight')
plt.close(fig)
...
xlabel = 'APHRODITE Return levels'
ylabel = 'Model Return levels'
title = 'Precip extreme return levels vs APHRODITE'
fname = 'vrcesm_prect_'+str(percentile)+'th_extremes_return_levels_vs_cordexsea_refAPHRODITE.pdf'
...
ylabel = 'Model Return levels'
title = 'Precip extreme return levels vs APHRODITE'
fname = 'vrcesm_prect_'+str(percentile)+'th_extremes_return_levels_refAPHRODITE.pdf'
