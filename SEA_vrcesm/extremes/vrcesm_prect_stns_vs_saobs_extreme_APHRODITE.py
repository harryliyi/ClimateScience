...
# set up APHRODITE data directories and filename
aphdir = "/scratch/d/dylan/harryli/obsdataset/APHRODITE/MA/"
aphfname = "APHRO_MA_025deg_V1101.1951-2007.nc"
...
iniaphdate = datetime.datetime.strptime(str(iniyear)+'-01-01', '%Y-%m-%d')
endaphdate = datetime.datetime.strptime(str(endyear)+'-12-31', '%Y-%m-%d')

# set up output directory and output log
outdir = "/scratch/d/dylan/harryli/cesm1/vrcesm/Analysis/vrseasia_AMIP_1979_to_2005/pre/extremes/SAOBS_countries/"+str(iniyear)+"-"+str(endyear)+"/vrseasia_regrid025x025_APHRODITE/"
...
# open APHRODITE file
aphdata = Dataset(aphdir+aphfname)
...
time_aph = aphdata.variables['time']
aphtime = netCDF4.num2date(time_aph[:], time_aph.units)
aphtime = pd.to_datetime(aphtime)
select_aphtime = (aphtime >= iniaphdate) & (aphtime <= endaphdate) & (~((aphtime.month == 2) & (aphtime.day == 29)))
...
aphlats = aphdata.variables['latitude'][:]
aphlons = aphdata.variables['longitude'][:]
...
aphpreall = np.array([], dtype=float)
...
for idx in range(len(stnids)):
    ...
    aphlat_idx = np.abs(aphlats - stnlat).argmin()
    aphlon_idx = np.abs(aphlons - stnlon).argmin()
    ...
    aphpre = aphdata.variables['precip'][select_aphtime, aphlat_idx, aphlon_idx]
    ...
    aphpre[np.isnan(saobspre[idx, :])] = np.NaN
    ...
    print("APHRODITE precip sum is: "+str(np.nansum(aphpre)))
    ...
    aphpreall = np.concatenate((aphpreall, aph_data['pre'][:]))
...
plt.title(str(percentile)+"th percentile: "+str(eraprepercent)+",  2000 obs needs threshold below: "+str(test_thresholds[(np.abs(test_menum - 2000)).argmin()]), fontsize=10)
plt.ylabel("Mean Excess")
plt.xlabel("Thresholds")
...
# plot for APHRODITE
aphpreall_nomiss = aphpreall[~np.isnan(aphpreall)]
aphpremax = np.amax(aphpreall_nomiss)
test_thresholds = np.arange(0, aphpremax, aphpremax/300)
...
for idx, ithres in enumerate(test_thresholds):
    aphpresub = aphpreall_nomiss[aphpreall_nomiss > ithres] - ithres
    test_me[idx] = np.mean(aphpresub)
    aphpresub_std = np.std(aphpresub)
    test_menum[idx] = len(aphpresub)
    test_meup[idx] = test_me[idx] + 1.96*(aphpresub_std/np.sqrt(len(aphpresub)))
    test_mebot[idx] = test_me[idx] - 1.96*(aphpresub_std/np.sqrt(len(aphpresub)))
...
aphprepercent = np.percentile(aphpreall_nomiss, percentile)

plt.plot(test_thresholds, test_me, linestyle='solid', linewidth=0.7, c='k')
plt.plot(test_thresholds, test_meup, linestyle='dashed', linewidth=0.7, c='k')
plt.plot(test_thresholds, test_mebot, linestyle='dashed', linewidth=0.7, c='k')

plt.suptitle("Mean residual life plot for APHRODITE over mainland SEA", fontsize=15)
plt.title(str(percentile)+"th percentile: "+str(aphprepercent)+",  2000 obs needs threshold below: "+str(test_thresholds[(np.abs(test_menum - 2000)).argmin()]), fontsize=10)
plt.ylabel("Mean Excess")
plt.xlabel("Thresholds")
plt.savefig(outdir+"aph_prect_"+str(percentile)+"th_mean_residual_life_plot_mainlandSEA.pdf")

plt.clf()
plt.plot(test_thresholds, test_menum, linestyle='solid', linewidth=0.7, c='k')

plt.suptitle("Number of Excess for APHRODITE over mainland SEA", fontsize=15)
plt.title(str(percentile)+"th percentile: "+str(aphprepercent)+",  2000 obs needs threshold below: "+str(test_thresholds[(np.abs(test_menum - 2000)).argmin()]), fontsize=10)
plt.ylabel("Number of  Excess")
plt.xlabel("Thresholds")
plt.savefig(outdir+"aph_prect_"+str(percentile)+"th_excess_number_plot_mainlandSEA.pdf")
...
aphpresub = aphpreall_nomiss[aphpreall_nomiss > aphprepercent] - aphprepercent
...
for ibins in np.arange(1.*prelen/3, 1.*prelen-2, 1.):  # (1.*prelen-1.*prelen/3)/1000):
    ...
    popt, pcov, tempscore = kstest(aphpresub, ibins)
    if tempscore.pvalue > aphpre_ks:
        aphpre_ks = tempscore.pvalue
        aphpre_best = popt
        aphpre_bestcov = pcov
        aphpre_nbins = ibins
    print(tempscore)
...
aph_return = np.zeros(len(year_returns))
...
for idx, iyear in enumerate(year_returns):
    ...
    if (aphpre_best[0] != 0):
        aph_return[idx] = aphprepercent + aphpre_best[1] / aphpre_best[0] * ((m*obsrate) ** aphpre_best[0] - 1)
    else:
        aph_return[idx] = aphprepercent + aph_fit[2] * math.log(m*obsrate)
...
plt.plot(year_returns, aph_return, c='c', label='APHRODITE')
...
plt.plot(sa_return, aph_return, c='c', label='APHRODITE')
plt.legend(loc='upper left')
