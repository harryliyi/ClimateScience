...
inicordate = datetime.datetime.strptime(str(iniyear)+'-01-01', '%Y-%m-%d')
endcordate = datetime.datetime.strptime(str(endyear)+'-12-31', '%Y-%m-%d')

# set up output directory and output log
outdir = "/scratch/d/dylan/harryli/cesm1/vrcesm/Analysis/vrseasia_AMIP_1979_to_2005/pre/extremes/SAOBS_countries/"+str(iniyear)+"-"+str(endyear)+"/vrseasia_regrid025x025_CORDEX-SEA/"
...
corvar, cortime, corlats, corlons = readcordex(varname, iniyear, endyear, project, modelname, frequency, latbounds, lonbounds)
...
corpreall = np.array([], dtype=float)
...
for idx in range(len(stnids)):
    ...
    corlat_idx = np.abs(corlats - stnlat).argmin()
    corlon_idx = np.abs(corlons - stnlon).argmin()
    ...
    corpre = corvar[:, corlat_idx, corlon_idx]
    ...
    corpre[np.isnan(saobspre[idx, :])] = np.NaN
    ...
    print("CORDEX-SEA precip sum is: "+str(np.nansum(corpre)))
    ...
    corpreall = np.concatenate((corpreall, cor_data['pre'][:]))
...
# plot for CORDEX
corpreall_nomiss = corpreall[~np.isnan(corpreall)]
corpremax = np.amax(corpreall_nomiss)
test_thresholds = np.arange(0, corpremax, corpremax/300)
...
for idx, ithres in enumerate(test_thresholds):
    corpresub = corpreall_nomiss[corpreall_nomiss > ithres] - ithres
    test_me[idx] = np.mean(corpresub)
    corpresub_std = np.std(corpresub)
    test_menum[idx] = len(corpresub)
    test_meup[idx] = test_me[idx] + 1.96*(corpresub_std/np.sqrt(len(corpresub)))
    test_mebot[idx] = test_me[idx] - 1.96*(corpresub_std/np.sqrt(len(corpresub)))
...
corprepercent = np.percentile(corpreall_nomiss, percentile)

plt.plot(test_thresholds, test_me, linestyle='solid', linewidth=0.7, c='k')
plt.plot(test_thresholds, test_meup, linestyle='dashed', linewidth=0.7, c='k')
plt.plot(test_thresholds, test_mebot, linestyle='dashed', linewidth=0.7, c='k')

plt.suptitle("Mean residual life plot for CORDEX-SEA over mainland SEA", fontsize=15)
plt.title(str(percentile)+"th percentile: "+str(corprepercent)+",  2000 obs needs threshold below: "+str(test_thresholds[(np.abs(test_menum - 2000)).argmin()]), fontsize=10)
plt.ylabel("Mean Excess")
plt.xlabel("Thresholds")
plt.savefig(outdir+"cor_prect_"+str(percentile)+"th_mean_residual_life_plot_mainlandSEA.pdf")

plt.clf()
plt.plot(test_thresholds, test_menum, linestyle='solid', linewidth=0.7, c='k')

plt.suptitle("Number of Excess for CORDEX-SEA over mainland SEA", fontsize=15)
plt.title(str(percentile)+"th percentile: "+str(corprepercent)+",  2000 obs needs threshold below: "+str(test_thresholds[(np.abs(test_menum - 2000)).argmin()]), fontsize=10)
plt.ylabel("Number of  Excess")
plt.xlabel("Thresholds")
plt.savefig(outdir+"cor_prect_"+str(percentile)+"th_excess_number_plot_mainlandSEA.pdf")
...
corpresub = corpreall_nomiss[corpreall_nomiss > corprepercent] - corprepercent
...
for ibins in np.arange(1.*prelen/3, 1.*prelen-2, 1.):  # (1.*prelen-1.*prelen/3)/1000):
    ...
    popt, pcov, tempscore = kstest(corpresub, ibins)
    if tempscore.pvalue > corpre_ks:
        corpre_ks = tempscore.pvalue
        corpre_best = popt
        corpre_bestcov = pcov
        corpre_nbins = ibins
    print(tempscore)
...
cor_return = np.zeros(len(year_returns))
...
for idx, iyear in enumerate(year_returns):
    ...
    if (corpre_best[0] != 0):
        cor_return[idx] = corprepercent + corpre_best[1] / corpre_best[0] * ((m*obsrate) ** corpre_best[0] - 1)
    else:
        cor_return[idx] = corprepercent + cor_fit[2] * math.log(m*obsrate)
...
plt.plot(year_returns, cor_return, c='c', label='CORDEX-SEA')
...
plt.plot(sa_return, cor_return, c='c', label='CORDEX-SEA')
plt.legend(loc='upper left')
