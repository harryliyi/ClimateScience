...
# setup directory
############################################################################
outdir = '/scratch/d/dylan/harryli/cesm1/vrcesm/Analysis/vrseasia_AMIP_1979_to_2005/pre/VRseasia_vs_CORDEX_SEA/TRMM/extremes/mainSEA/overland/all/'
...
print('Plot the precip histogram for model and TRMM')
fig = plt.figure()
...
fname = 'vrcesm_all_prect_hist_vs_cordex_refTRMM.pdf'
...
model_ts1 = model_ts1[model_ts1 > premin]
model_ts2 = model_ts2[model_ts2 > premin]
model_ts3 = model_ts3[model_ts3 > premin]
model_ts4 = model_ts4[model_ts4 > premin]

cordex_ts1 = cordex_ts1[cordex_ts1 > premin]
cordex_ts2 = cordex_ts2[cordex_ts2 > premin]
cordex_ts3 = cordex_ts3[cordex_ts3 > premin]
cordex_ts4 = cordex_ts4[cordex_ts4 > premin]

obs_ts = obs_ts[obs_ts > premin]
print(len(obs_ts))
print(obs_ts)
...
fname = 'vrcesm_prect_over_'+str(premin)+'mm_hist_vs_cordex_refTRMM.pdf'

plt.suptitle(title, fontsize=9, y=0.95)
plt.savefig(outdir+fname, bbox_inches='tight')
plt.close(fig)

# test the mean residual life plot for TRMM
...
fname = 'TRMM_prect_'+str(percentile)+'th_mean_residual_life_plot.pdf'
plt.savefig(outdir+fname, bbox_inches='tight')
plt.close(fig)
...
title = 'Estimated parameter (Xi) for TRMM'
plt.suptitle(title, fontsize=10, y=0.95)

fname = 'TRMM_prect_'+str(percentile)+'th_xi_plot.pdf'
plt.savefig(outdir+fname, bbox_inches='tight')
plt.close(fig)
...
title = 'Estimated parameter (sigma) for TRMM'
plt.suptitle(title, fontsize=10, y=0.95)

fname = 'TRMM_prect_'+str(percentile)+'th_sigma_plot.pdf'
plt.savefig(outdir+fname, bbox_inches='tight')
plt.close(fig)
...
xlabel = 'TRMM Return levels'
ylabel = 'Model Return levels'
title = 'Precip extreme return levels vs TRMM'
fname = 'vrcesm_prect_'+str(percentile)+'th_extremes_return_levels_vs_cordexsea_refTRMM.pdf'
...
ylabel = 'Model Return levels'
title = 'Precip extreme return levels vs TRMM'
fname = 'vrcesm_prect_'+str(percentile)+'th_extremes_return_levels_refTRMM.pdf'
